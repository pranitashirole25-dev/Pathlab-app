...
class ScorecardService {
     ...
     */
    static async calculateAndApplyPenalty(appointmentId, scheduledTime, actualArrivalTime) {
        ...
        try {
            ...
            const apptResult = await (0, db_1.query)('SELECT phlebotomist_id FROM appointments WHERE id = $1', [appointmentId]);
            const phlebotomistId = apptResult.rows[0]?.phlebotomist_id;
            if (!phlebotomistId)
                throw new Error('Phlebotomist not found for appointment');
            ...
            const scorecardResult = await (0, db_1.query)(`UPDATE scorecards 
         SET penalties = penalties + $1, updated_at = NOW() 
         WHERE employee_id = $2 AND month_start = $3 
         RETURNING *`, [penaltyAmount, phlebotomistId, monthStart]);
            ...
            if (scorecardResult.rowCount === 0) {
                await (0, db_1.query)(`INSERT INTO scorecards (employee_id, month_start, incentive_pool, penalties) 
            VALUES ($1, $2, 0, $3)`, [phlebotomistId, monthStart, penaltyAmount]);
            }
     ...
     */
    static async recordArrival(appointmentId) {
        const arrivalTime = new Date();
        // Update appointment status to IN_PROGRESS
        const result = await (0, db_1.query)(`UPDATE appointments 
       SET status = 'IN_PROGRESS', actual_start_time = $1 
       WHERE id = $2 RETURNING scheduled_start_time`, [arrivalTime, appointmentId]);
...
exports.default = ScorecardService;

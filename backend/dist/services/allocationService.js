...
class AllocationService {
     ...
     */
    static async autoAllocateHomeBooking(bookingId) {
        try {
            // 1. Fetch booking details
            const bookingResult = await (0, db_1.query)('SELECT * FROM bookings WHERE id = $1', [bookingId]);
            ...
            const phlebotomistResult = await (0, db_1.query)(`
        SELECT u.id, u.phone 
        FROM users u
      ...
      `, [today]);
            ...
            const apptResult = await (0, db_1.query)(`INSERT INTO appointments (booking_id, phlebotomist_id, scheduled_start_time, status) 
         VALUES ($1, $2, $3, 'SCHEDULED') RETURNING id`, [bookingId, assignedStaff.id, scheduledStartTime]);
            console.log(`Auto-allocated booking ${bookingId} to staff ${assignedStaff.id}`);
            // 4. Send WhatsApp Notification to Staff
            await whatsappService_1.default.sendOTP(assignedStaff.phone, 'NEW_APPOINTMENT_ALLOCATED');
...
exports.default = AllocationService;
